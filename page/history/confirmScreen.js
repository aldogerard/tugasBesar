...
export default class ConfirmScreen extends Component {
  ...
  getData() {
    ...
    data.includes("@")
      ? getDocs(query(collection(db, "registrasi"), where("email", "==", data), where("status", "==", "berhasil"))).then((docSnap) => {
          let users = [];
          docSnap.forEach((doc) => {
      ...
      : getDocs(query(collection(db, "registrasi"), where("tipeRS", "==", data), where("status", "==", "berhasil"))).then((docSnap) => {
          let users = [];
          docSnap.forEach((doc) => {
            users.push({ ...doc.data(), id: doc.id });
          });
