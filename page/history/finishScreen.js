...
export default class FinishScreen extends Component {
  ...
  getData() {
    ...
    data.includes("@")
      ? getDocs(query(collection(db, "registrasi"), where("email", "==", data), where("status", "==", "selesai"))).then((docSnap) => {
          let users = [];
          docSnap.forEach((doc) => {
      ...
      : getDocs(query(collection(db, "registrasi"), where("tipeRS", "==", data), where("status", "==", "selesai"))).then((docSnap) => {
          let users = [];
          docSnap.forEach((doc) => {
            users.push({ ...doc.data(), id: doc.id });
          });
